...
tstats.edge_age.nf.s.rural <- c(formation_stats$rural$edge %>% filter(contact_location == "School" ) %>% pull(edges_adj_age), # edge
                            (formation_stats$rural$nf.age.grp %>% filter(contact_location == "School" ) %>% pull(nf.ag))[-1]  # nodefactor, remove the oldest group, which has the lowest number of nodes (<10)
                            )

## Work, rural
tstats.edge_age.nf.w.rural <- c(formation_stats$rural$edge %>% filter(contact_location == "Work" ) %>% pull(edges_adj_age), # edge
                            (formation_stats$rural$nf.age.grp %>% filter(contact_location == "Work" ) %>% pull(nf.ag))[-c(1)]  # nodefactor, remove the first two youngest groups , which has the lowest number of nodes (<10)
                            )
...
# 20231107 reach here, start by supplying 0 to low degree levels.
...
est.s.edges.nf <- netest(nw_rural, 
                      ...
                      target.stats = tstats.edge_age.nf.s.rural, # we are passing low values. Its trying to find the value for the 6 one.
...
#                    dynamic = T, 
#                set.control.ergm = control.simulate.formula(MCMC.burnin = 1e5),
#                nwstats.formula = ~edges + nodefactor("age.grp" , levels = 1:5), 
#                keep.tedgelist = TRUE
#                )
...
est.w.edges.nf <- netest(nw_rural, 
                      formation = ~edges + nodefactor("age.grp" , levels = 3:6) , 
...
dx.w.edges.nf<- netdx(est.w.edges.nf, nsims = 20, ncores = 8, 
                   ...
                   dynamic = T, 
               set.control.ergm = control.simulate.formula(MCMC.burnin = 1e5),
               nwstats.formula = ~edges + nodefactor("age.grp" , levels = 3:6), 
               keep.tedgelist = TRUE
               )

dx.w.edges.nf 

# simulation adding an extra term
dx.w.edges.nf.nm_sim <- netdx(est.w.edges.nf, nsims = 20, ncores = 8, 
                   nsteps = 1000, 
                   dynamic = T, 
               set.control.ergm = control.simulate.formula(MCMC.burnin = 1e5),
               nwstats.formula = ~edges + nodefactor("age.grp" , levels = 3:6) + nodematch("age.grp" , levels = 3:6, diff = T), 
               keep.tedgelist = TRUE
               )
...
c(tstats.edge_age.nf.w.rural, (formation_stats$rural$nm.age.grp %>% filter(contact_location == "Work" ) %>% pull(nm.ag))[3:6])
