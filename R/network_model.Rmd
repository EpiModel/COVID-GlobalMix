...
# 
# formation.edge.nf_ag.nm_1 <- # dropping the 4th age group which has the best fitf
...
c(tstats.edge_age.nf.s.rural, 
  c((formation_stats$rural$nm.age.grp %>% filter(contact_location == "School" ) %>% pull(nm.ag)
                               )[-c(1,3:5)],0,0,0,0) # the value of the last three groups are <20, we pass 0 to them and the 6th grp, which wasn't observed
...
est.s.edges.nf.nm <- netest(nw_rural, 
                      formation = formation.edge_age.nf.nm , 
                      target.stats = tstats.edge.nf_nm.work.rural, 
                      coef.diss =  diss.work.rural,
                 set.control.ergm = control.ergm(MCMLE.maxit = 500)
                 )
# talk to Sam, this model cannot be
...
tstats.edge_age.nf.w.rural <- c(formation_stats$rural$edge %>% filter(contact_location == "Work" ) %>% pull(edges_adj_age), # edge
                            c(0, (formation_stats$rural$nf.age.grp %>% filter(contact_location == "Work" ) %>% pull(nf.ag))[-c(1,2)] ) # excluding the 1st category, considering it as the default reference, passing zero to the 2nd category given low target stat
                            )
...
c(tstats.edge_age.nf.w.rural, 
  c(0,(formation_stats$rural$nm.age.grp %>% filter(contact_location == "Work" ) %>% pull(nm.ag)
                               )[-c(1,2)])
  )# excluding the 1st category, considering it as the default reference, passing zero to the 2nd category given low target stat
...
est.w.edges.nf.nm <- netest(nw_rural, 
                      formation = formation.edge_age.nf.nm , 
                      target.stats = c(tstats.edge.nf_nm.work.rural[-length(tstats.edge.nf_nm.work.rural)],0), 
                      coef.diss =  diss.work.rural,
                 set.control.ergm = control.ergm(MCMLE.maxit = 500)
                 )
