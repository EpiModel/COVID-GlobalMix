...
india_mix <- india_participant %>%
  ...
  # characterization of home/nonhome status
   ...
   ## Weighing in household membership in determining contact location - tabulating contact_location and hh_membership, we found there are 58,1,3 household member contacts in the nonhome, school, and work layers. We reclassify these as home contacts
  mutate(
...
# Evaluate characteristics of non-members at home
test <- 
india_mix %>% filter(hh_membership == "Non-member" & contact_location =="Home")

apply(test[,c(17:42)], 2, table)
...
For the two contacts (shown below) that occurred at both work (location 3 in REDCap) and school (location 2), we categorized them as at school, considering both the participants and contacts were at school age.
...
india_mix %>% filter(known_contact == ">10 yrs" & contact_location %in% c( "School", "Work" )
    ...
    mutate(known_duration = (participant_ageyr - 10)/2) %>% filter(known_duration<0) %>% select(participant_ageyr, known_contact, known_duration)
