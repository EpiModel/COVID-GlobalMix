...
# Check with Sam about the next steps
...
contact_freq_site(india_mix. = india_mix , 
 ...
 # compare the number of rows in different layer to see if the nonhome differs from the others.
contact_count_rural$age.grp_mix_degree$age.grp1_2 %>% filter(contact_location == "Nonhome") %>% nrow()
contact_count_rural$age.grp_mix_degree$age.grp1_2 %>% filter(contact_location == "Home") %>% nrow()
contact_count_rural$age.grp_mix_degree$age.grp1_2 %>% filter(contact_location == "School") %>% nrow()
contact_count_rural$age.grp_mix_degree$age.grp1_2 %>% filter(contact_location == "Work") %>% nrow()


contact_count_rural$age.grp_mix_degree$age.grp1_1 <- contact_count_rural$age.grp_mix_degree$age.grp2_2 <- contact_count_rural$age.grp_mix_degree$age.grp3_3 <- contact_count_rural$age.grp_mix_degree$age.grp4_4 <- 
  contact_count_rural$age.grp_mix_degree$age.grp5_5 <- contact_count_rural$age.grp_mix_degree$age.grp6_6 <- NULL

contact_count_rural$age.grp_mix_degree


mix_prop <- # discuss with Sam
function(mix_degree, age.grp.a, age.grp.b, contact_loc){

dir_1 <-   paste0("age.grp", age.grp.a, "_",age.grp.b) # contact direction between participant in age group a and contact in age group b
dir_2 <-   paste0("age.grp", age.grp.b, "_", age.grp.a)# contact direction between participant in age group b and contact in age group a 
...
# if no cat, skip the below calculation
if(
sum(df_dir_1$participant_age == age_cats[age.grp.a]) >0 # if there is no participant of this age group b, an NA will be returned for the proportion
){
glm_nmix_dir_1 <-
df_dir_1 %>% 
...
prop_dir_1 <- glm_nmix_dir_1$coefficients %>% as.data.frame() %>% rownames_to_column(var = "age.grp") %>% 
  mutate(age.grp = str_replace(age.grp, "participant_age", "")) %>% filter(age.grp == age_cats[age.grp.a]) %>% pull(Estimate) %>% expit()
} else{
  prop_dir_1 <- NA
}

if(
sum(df_dir_2$participant_age == age_cats[age.grp.b])>0 # if there is no participant of this age group b, an NA will be returned for the proportion
){
...
prop_dir_2 <- glm_nmix_dir_2$coefficients %>% as.data.frame() %>% rownames_to_column(var = "age.grp") %>% 
  mutate(age.grp = str_replace(age.grp, "participant_age", "")) %>% filter(age.grp == age_cats[age.grp.b]) %>% pull(Estimate) %>% expit() 
} else{
  prop_dir_2 <- NA 
}
  
data.frame(matrix(c(prop_dir_1, prop_dir_2), nrow =1, ncol =2)) %>% setNames(c(dir_1, dir_2)) # add two more column here to 
...
function(mix_degree_site){
 ...
 # i=2 # issue with school layer  

for (i in 1:4) {
  print(layers[i])

#  evaluate how no data in > 60y is adjusted 
...
for (j in 2:6) {
  ...
  print(directional_prop)
   ...
   mix_matrix[1, j] <-  directional_prop %>% as.numeric() %>% mean(., na.rm = T)
 
 
}

## participant age.group == "10-19y"    
for (j in 3:6) {
  ...
  print(directional_prop)
   ...
   mix_matrix[2, j] <-  directional_prop %>% as.numeric() %>% mean(., na.rm = T)
  
}

## participant age.group == "20-29y"
for (j in 4:6) {
  ...
  print(directional_prop)
   ...
   mix_matrix[3, j] <-  directional_prop %>% as.numeric() %>% mean(., na.rm = T)
  
  
}

## participant age.group == "30-39y"
for (j in 5:6) {
   directional_prop <- 
  ...
  print(directional_prop)
   ...
   mix_matrix[4, j] <-  directional_prop %>% as.numeric() %>% mean(., na.rm = T)
}

## participant age.group == "40-59y"
  directional_prop <- 
   mix_prop(mix_degree = mix_degree_site, age.grp.a = 5, age.grp.b = 6, contact_loc = layers[i]) 
  
  print(directional_prop)
  
   mix_matrix[5, 6] <-  directional_prop %>% as.numeric() %>% mean(., na.rm = T)
