...
india_mix.site %>% 
 ...
 # Replicate each observation/row w/ same.age.grp == 2 twice and recode these observations' same.age.grp to 1; for observations w/ same.age.grp = 1/0, we treat them as-is
   filter(same.age.grp==2) %>% slice(rep(1:n(), each = 2)) %>% mutate(same.age.grp=1) %>% 
    rbind(., same.age.grp_count %>% filter(same.age.grp !=2)
          ) %>% rename(same.age.grp = same.age.grp) # reanme to a similar name
...
names(age.grp_mix_degree)<- colnames(age.grp_mix %>% select(age.grp1_1:age.grp6_6)) # adding mixing as name to the list
 ...
 out <-list(contact_count, same.age.grp_count, age.grp_mix_degree); names(out) <- c("contact_degree", "sameage.grp.degree", "age.grp_mix_degree")
...
# nodemix - porportion of edges of specific mixing among all the edges
