...
india_mix <- india_participant %>%
  ...
  # Note unmute this after analysis replicated contact
  # mutate(
...
location_labs <- as.character(sjlabelled::get_label(mix))
...
```{r, echo=F, warning=F, message=FALSE}
n_participant_rural <- india_participant %>% filter(study_site == "Rural") %>% pull(rec_id) %>% unique() %>% length()
n_participant_urban <- india_participant %>% filter(study_site == "Urban") %>% pull(rec_id) %>% unique() %>% length()
par(mfrow = c(2, 2), mar=c(5, 4, 4, 2) + 0.5) 
barplot(nonhome_dup_rural$location_freq, main = "Rural", ylab = "Total frequency of the whole population", ylim = c(0, 1100),
        las = 2, 
        cex.names = 1)
barplot(nonhome_dup_urban$location_freq, main = "Urban", ylab = "Total frequency of the whole population", ylim = c(0,1100),
        las = 2, cex.names = 1)

barplot(nonhome_dup_rural$location_freq/n_participant_rural, main = "Rural", ylab = "Mean contacts per participant", ylim = c(0, 2),
        las = 2, 
        cex.names = 1)
barplot(nonhome_dup_urban$location_freq/n_participant_urban, main = "Urban", ylab = "Mean contacts per participant", ylim = c(0,2),
        las = 2, cex.names = 1)
