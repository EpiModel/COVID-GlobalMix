...
layer_assoc_rural <- assoc_btw_layers(contact_count_wide = contact_count_wide_rural,  output_type = "models")
...
layer_assoc_urban <- assoc_btw_layers(contact_count_wide = contact_count_wide_urban,  output_type = "models")


contact_count_wide_rural$Home %>% mean()/2

exp(layer_assoc_rural$h_w$coefficients[1]+layer_assoc_rural$h_w$coefficients[2]*0)/2
exp(layer_assoc_rural$h_w$coefficients[1]+layer_assoc_rural$h_w$coefficients[2]*1)/2


# function to characterize the target statistics based on coefficients of Poisson regression
effect_oth_layers <- 
  function(
    layer_assoc
    ){
...
# Network statistics on one layer has on the other layer
rbind(
## rural network
...
### effect of the work layer on school layer
effect_oth_layers(
 layer_assoc = layer_assoc_rural$s_w
),
...
### effect of the work layer on school layer
effect_oth_layers(
 layer_assoc = layer_assoc_urban$s_w
),
...
### effect of the work layer on nonhome layer
effect_oth_layers(
 layer_assoc = layer_assoc_urban$nh_s
)
) %>% data.frame()%>% 
  mutate(association = c("h_w, rural",
                         "s_w, rural", 
                         "w_s, rural",
                         "nh_w, rural", 
                         "s_w, urban", 
