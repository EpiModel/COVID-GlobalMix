...
assoc_btw_layers <- function(contact_count_wide, category_type){
...
glm_h_nh <- # For an intercept only model, the coefficient is positive in in valid so we add the intercept back
contact_count_wide %>% 
  glm(Home~ Nonhome_cat, data=., family = poisson(link=log)) %>% summary()
...
output[[3]] <- rbind(glm_h_wk$coefficients, NA, glm_h_nh$coefficients, NA, glm_h_sch$coefficients, NA, glm_h_nh_sch_wk$coefficients)
...
assoc_btw_layers(contact_count_wide = contact_count_wide_rural, category_type = "binary")

assoc_btw_layers(contact_count_wide = contact_count_wide_urban, category_type = "nonbinary")


# Home ~ Work
# Home ~ School, 
# Home ~ Nonhome + school + work
# we domn't model cor(school. home). We include correlation between work & home
