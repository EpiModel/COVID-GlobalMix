...
get_subnet <- function(el_cuml, node) {
  subnet <- numeric(0)
  new <- node
  while (new > 0) {
    sub_el <- dplyr::filter(head %in% new | tail %in% new)
    subnet <- unique(c(subnet, sub_el$head, sub_el$tail))
...
get_subnet <- function(connected, nodes) {
  subnet <- numeric(0)
  new <- nodes
  while (length(new) > 0) {
    new <- unlist(connected[new])
...
get_subnets_el <- function(connected, el) {
  ...
  i <- 1
  while (nrow(el) > 0) {
    subnets[i] <- get_subnet(connected, el$head) # by def: head <-> tail
    el <- dplyr::filter(!head %in% subnets[i])
    i <- i + 1
  }
  subnets
}
...
#   connected_sub[subnet] <- list(subnet)
# }
